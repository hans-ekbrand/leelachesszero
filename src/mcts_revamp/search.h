...
class SearchWorker_revamp {
 public:
  SearchWorker_revamp(Search_revamp* search, Node_revamp* worker_root)
      : search_(search), history_(search_->played_history_), worker_root_(worker_root) {}
 ...
 private:
  ...
  std::vector<float> q_to_prob(std::vector<float> Q, int d, float multiplier);
