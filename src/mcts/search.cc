...
*/
    ...
    int npick = std::min(64, params_.GetMiniBatchSize() - minibatch_size);
...
bool SearchWorker::PickNodeToExtendRec(Node *node, Node::Iterator second_best_edge, bool is_root_node, uint16_t depth, int best_node_n, int collision_limit, int& minibatch_size, int& collision_events_left, int& collisions_left, int& number_out_of_order, int& npick) {
    ...
    if (second_best_edge2) {
      ...
      quit = PickNodeToExtendRec(second_best_edge2.GetOrSpawnNode(/* parent */ node), second_best_edge, false, depth, best_node_n, collision_limit, minibatch_size, collision_events_left, collisions_left, number_out_of_order, npick);
      history_.Pop();
    }
    return quit;
